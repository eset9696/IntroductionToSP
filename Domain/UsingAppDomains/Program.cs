...
namespace UsingAppDomains
{
	internal static class Program
	{
		...
		/// </summary>
		[STAThread]
		[LoaderOptimization(LoaderOptimization.MultiDomain)] // Для доступа к исполняемому коду друг друга
		static void Main()
		{
			...
			TextWindowForm = Activator.CreateInstance(textWindowAsm.GetType("TextWindow.MainForm"), new object[] { drawerAsm.GetModule("TextDrawer.exe"), DrawerForm}) as Form;
			//TextWindowForm = Activator.CreateInstance(textWindowAsm.GetType("TextWindow.MainForm")) as Form;
			(new Thread(new ThreadStart(RunVisualizer))).Start();
			(new Thread(new ThreadStart(RunDrawer))).Start();

			drawer.DomainUnload += new EventHandler(DrawerDomainUnload);
		}

		static void DrawerDomainUnload(object sender, EventArgs e)
		{
